...
public class Skill2CmdPlayableAdapter : CmdPlayableAdapter
{
    ...
    private AttackLinkSkillData m_AttackLink = null;
    private AttackLinkItemData CurAtkLinkItemData => m_AttackLink.GetData((int)m_StepIndex);

    protected override void OnDestroy()
    {
        PlayableAdapter.Destroy(m_PlayableAdapter);
        GameClassPoolMgr.Instance.Push(m_AttackLink);
        base.OnDestroy();
    ...
    public override void OnPoolInit(CustomPoolData userData)
    {
        ...
        m_AttackLink = GameClassPoolMgr.Instance.Pull<AttackLinkSkillData>();
        m_AttackLink.InitData(arrData.arrParams);



        m_LastExecuteTime = ABBUtil.GetGameTimeSeconds();

        m_PlayableAdapter = m_Graph.CreateClipPlayableAdapter(CurAtkLinkItemData.clipID);
    ...
    private void CmdEnd()
    {
        ...
        var to = m_Graph.CreateClipPlayableAdapter(CurAtkLinkItemData.clipID);
        from.Complete();
        DisconnectRootAdapter();
